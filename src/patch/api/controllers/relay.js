...
const SESSION_NAME = 'CustomRelaySession';
...
function getStreams(req, res, next) {
    ...
    this.sessions.forEach(function (session, id) {
        if (session.constructor.name !== SESSION_NAME) {
            return;
        }
...
function getStreamByID(req, res, next) {
    const relaySession = Array.from(this.sessions.values()).filter(
        (session) =>
            session.constructor.name === SESSION_NAME &&
...
async function pushStream(req, res, next) {
    ...
    let app = req.body.app;
    let name = req.body.name;
    let loop = req.body.loop || 1;
    let params = req.body.params ? req.body.params : {};

    if (url && app && name) {
        process.nextTick(() => this.nodeEvent.emit('relayPush', url, app, name, loop, params));
